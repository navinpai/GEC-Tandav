...
Autocompleter.Base = new Class({
	...
	onCommand: function(e, mouse) {
		...
		if (e.key && !e.shift) {
			...
			if(window.ie && e.code==188){
				this.customSelect(this.element.value);
				e.stop();
				return;
			}
			...
			case 'backspace':
				...
				if(this.element.value=="" && this.element.getParent().getPrevious()){
					this.element.getParent().getPrevious().remove();
					e.stop();
				}		
				return;
	...
	choiceSelect: function(el) {
		var li = new Element('li', {'class':'addedTag'} ).setHTML(el.inputValue).injectBefore(this.element.getParent());
		var img = new Element('span', {'class':'tagRemove', events: { click: function() { this.getParent().remove();} }}).setHTML('x').injectInside(li);
		var input = new Element('input', {'type':'hidden', 'value':el.inputValue, 'name':'tags[]'}).injectInside(li);
	...
	customSelect: function(value) {
		var li = new Element('li', {'class':'addedTag'} ).setHTML(value).injectBefore(this.element.getParent());
		var img = new Element('span', {'class':'tagRemove', events: { click: function() { this.getParent().remove();} }}).setHTML('x').injectInside(li);
		var input = new Element('input', {'type':'hidden', 'value':value, 'name':'tags[]'}).injectInside(li);
...
Autocompleter.Ajax.Base = Autocompleter.Base.extend({
	...
	query: function(){
		...
		var current = this.element.value.split(",");
		data[this.options.postVar] = current.getLast().trim();
